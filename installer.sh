...
. "${path_script}/install/inquirer.sh"
...
waInstall() {
    ...
    . "${path_script}/bin/winapps" install
}
...
waConfigureApps() {
    ...
    ${SUDO} cp "${path_script}/bin/winapps" "${BIN_PATH}/winapps"
    COUNT=0
...
waConfigureWindows() {
    ...
    if [ ${USEDEMO} != 1 ]; then
        ...
        ${SUDO} cp "${path_script}/icons/windows.svg" "${SYS_PATH}/icons/windows.svg"
        echo "[Desktop Entry]
Name=Windows
